...
func (s *Subscriber) read(ctx context.Context, topic string, readChannel chan<- string, logFields watermill.LogFields) {
	wg := &sync.WaitGroup{}
	defer func() {
		wg.Wait()
		close(readChannel)
	...
	for {
		...
		default:
			// 获取多个
			rzs, err = s.client.BLPop(ctx, 1*time.Second, topic).Result()
			if err == redis.Nil {
				continue
			} else if err != nil {
...
func (h *messageHandler) processMessage(ctx context.Context, topic string, read string, messageLogFields watermill.LogFields) error {
	srcKey := getKey(topic, read)
